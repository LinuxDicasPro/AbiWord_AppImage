...
APP=abiword
BIN="$APP"
DEPENDENCES="at-spi2-core cairo enchant fontconfig fribidi gcc-libs gdk-pixbuf2 glib2 glibc goffice gtk3 hicolor-icon-theme libgsf libical libjpeg-turbo libpng librsvg libx11 libxml2 libxslt pango rasqal readline redland wv zlib libwmf libwpg libwps link-grammar telepathy-glib adwaita-icon-theme shared-mime-info"
...
lib_browser_launcher="gio-launch-desktop libdl.so libpthread.so librt.so libasound.so libX11-xcb.so"
...
function _remove_more_bloatwares() {
    ...
    rm -rf ./"$APP".AppDir/.junest/usr/lib64/systemd
    rm -rf ./"$APP".AppDir/.junest/usr/lib64/tmpfiles.d
}
...
$HERE/.local/share/junest/bin/junest -n -b "$BINDS" -- bash -c "/usr/bin/gdk-pixbuf-query-loaders --update-cache"
cd ..

rm depdeps*

ARCH=x86_64 ./appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 ./$APP.AppDir
mv ./AbiWord-x86_64.AppImage ./AbiWord-$VERSION-x86_64.AppImage
